...
## "미배치 유닛" 목록에 유닛 하나를 등록하는 입력창.
## 이름 / 모델 수 / 가로(mm) / 세로(mm, 비우면 원형) / 팀을 입력받는다.
## 패널 바깥을 클릭하면 취소된다.
...
func _build_ui() -> void:
	...
	_count_edit = _add_field(box, "모델 수")
...
func _on_confirm_pressed() -> void:
	var count := 1
	if _count_edit.text.is_valid_int():
		count = int(_count_edit.text)
