...
namespace TmgBoard
{
    ...
    public static class GameLoadRequest
    {
        ...
        /// "undo_history" 키를 통해서 온다 — SaveGame이 이제 항상 그 키를
        /// 채워서 저장하므로(2026-09-04부터, BoardManager.Save.cs의
        /// BuildFullStateTree(includeUndoHistory: true)) 이어하기하면 그
        /// 판의 되돌리기가 그대로 살아있다. 게임 도중 멀티 시작 전송만은
        /// 여전히 그 키를 안 채운다 — 별도 이유로 항상 지우는 게 맞기
        /// 때문에, BoardManager.MidGameHandoff.cs 참고). 그래서 이 필드는
        /// 파일 불러오기 경로에서는 항상 null로 남는다.
